...
def main():
    ...
    local_llm = ChatOllama(temperature=0, model="qwen2.5:0.5b-instruct")

    chain_local_llm = summary_prompt_template | local_llm

    response_local_llm = chain_local_llm.invoke(input={"information": information})

    # ── LOCAL LLM OUTPUT ─────────────────────────────────────────────────────

    # Print the response as formatted Markdown in PyCharm's terminal.
    # In Jupyter Notebook, you can use:
    # display(Markdown(response_local_llm.content))

    console.print(Markdown(response_local_llm.content))
